...
export async function getMessages(userIDFrom, userIDTo, numMessages=20){
    ...
    //Test the query ouput for eventual SQL
    try{
    const userToMsgsResults = await find("chat", `uid1='${userIDFrom}' AND uid2='${userIDTo}'`, "time DESC");
    const userFromMsgsResults = await find("chat", `uid2='${userIDTo}' AND uid1='${userIDFrom}'`, "time DESC");
    }
    catch(e){
        console.log(e);
    }
